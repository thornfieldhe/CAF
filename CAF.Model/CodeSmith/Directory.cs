...
namespace CAF.Model
{
	...
	{   
        ...
        #region 常量定义
        protected const string QUERY_COUNT = "SELECT COUNT(*) AS COUNT FROM Sys_Directories Where Status!=-1 ";
       ...
       public static DirectoryList GetAllByParentId(Guid parentId)
        ...
        public static DirectoryList Query(Expression<Func<IQueryable<Directory>, IQueryable<Directory>>> exp)
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {                
                var expc = new ExpConditions<Directory>();
                expc.Add(exp);
                var items = conn.Query<Directory>(string.Format("{0} {1} {2}", QUERY_GETAll, expc.Where(), expc.OrderBy())).ToList();
        ...
        /// <param name="exp">表达式</param>
        /// <returns></returns>
        public static int QueryCount(Expression<Func<IQueryable<Directory>, IQueryable<Directory>>> exp)
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {
                var expc = new ExpConditions<Directory>();
        ...
        /// <param name="exp">表达式</param>
        /// <returns></returns>
        public static bool Exists(Expression<Func<IQueryable<Directory>, IQueryable<Directory>>> exp)
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {
                var expc = new ExpConditions<Directory>();
