...
namespace CAF.Model
{
    ...
    public class WorkflowRuleList:CollectionBase<WorkflowRuleList,WorkflowRule>
    {
        ...
        public static WorkflowRuleList Query(Object dynamicObj, string query = " 1=1")
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {
                var items = conn.Query<WorkflowRule>(string.Format(QUERY, "Sys_WorkflowRules", query), dynamicObj).ToList();
        ...
        public static int QueryCount(Object dynamicObj, string query = " 1=1")
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {
                return conn.Query<int>(string.Format(COUNT, "Sys_WorkflowRules", query), dynamicObj).Single();
            }
        }
        ...
        public static bool Exists(Object dynamicObj, string query = " 1=1")
        {
            using (IDbConnection conn = SqlService.Instance.Connection)
            {
               return conn.Query<int>(string.Format(COUNT, "Sys_WorkflowRules", query), dynamicObj).Single()>0;
            }
        }
    }
}
