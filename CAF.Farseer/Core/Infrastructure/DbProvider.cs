...
using CAF.FS.Core.Client.MySql;
using CAF.FS.Core.Client.OleDb;
using CAF.FS.Core.Client.Oracle;
using CAF.FS.Core.Client.SqLite;
...
namespace CAF.FS.Core.Infrastructure
{
    ...
    public abstract class DbProvider
    {
        ...
        {
            var param = this.GetDbProviderFactory.CreateParameter();
            param.DbType = type;
            param.ParameterName = this.ParamsPrefix + name;
            param.Value = this.ParamConvertValue(valu, type);
        ...
        public DbParameter CreateDbParam(string name, object valu, bool output = false)
        {
            ...
            int len;
            var type = this.GetDbType(valu, out len);
            return this.CreateDbParam(name, valu, type, len, output);
        ...
        {
            int len;
            var type = this.GetDbType(valType, out len);
            return this.CreateDbParam(name, valu, type, len, output);
        }
        ...
        public IList<DbParameter> GetParameter<TEntity>(TEntity entity) where TEntity : class,new()
        {
            ...
            foreach (var kic in map.MapList.Where(o => o.Value.FieldAtt.IsMap))
            {
                ...
                //  添加参数到列表
                lst.Add(this.CreateDbParam(kic.Value.FieldAtt.Name, obj));
            }
        ...
        public string CreateDbConnstring(int dbIndex = 0)
        {
            ...
            return this.CreateDbConnstring(dbInfo.UserID, dbInfo.PassWord, dbInfo.Server, dbInfo.Catalog,
