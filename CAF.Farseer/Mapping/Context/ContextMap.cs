...
namespace CAF.FS.Mapping.Context
{
    /// <summary>
    ...
    public class ContextMap
    {
        ...
        {
            this.Type = type;
            this.MapList = new Dictionary<PropertyInfo, SetState>();

            #region 类属性
            var attContext = this.Type.GetCustomAttributes(typeof(ContextAttribute), false);
            foreach (var attr in attContext.OfType<ContextAttribute>()) { this.ContextProperty = attr; }
            if (this.ContextProperty == null) { this.ContextProperty = new ContextAttribute(); }
            #endregion

            #region 变量属性

            // 遍历所有Set属性(表、视图、存储过程的名称),取得对应使用标记名称
            foreach (var setProperty in this.Type.GetProperties())
            {
                ...
                //添加属变量标记名称
                this.MapList.Add(setProperty, setState);
            }
            #endregion
        }
        ...
        {
            return this.MapList.Single(o => o.Key == setPropertyInfo);
        }
        ...
        {
            return this.MapList.Single(o => o.Key.PropertyType == setType && o.Key.Name == propertyName);
        }
    }
}