...
namespace CAF.FS.Mapping.Context
{
    ...
    public class FieldMap
    {
        ...
        {
            this.Type = type;
            this.MapList = new Dictionary<PropertyInfo, FieldState>();

            #region 变量属性

            // 循环Set的字段
            foreach (var fieldProperty in this.Type.GetProperties())
            {
                // 获取字段的特性
                ...
                if (fieldState.FieldAtt.IsMap && fieldState.FieldAtt.IsPrimaryKey) { this.PrimaryState = new KeyValuePair<PropertyInfo, FieldState>(fieldProperty, fieldState); } else { fieldState.FieldAtt.IsPrimaryKey = false; }


                //添加属变量标记名称
                this.MapList.Add(fieldProperty, fieldState);
            }
        ...
        {
            return string.IsNullOrEmpty(fieldName) ? this.MapList.FirstOrDefault(o => o.Value.FieldAtt.IsPrimaryKey) : this.MapList.FirstOrDefault(o => o.Key.Name == fieldName);
        }
    }
}